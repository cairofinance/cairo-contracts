...
contract MasterChefTimelock is Timelock {
    ...
    struct TransferCairoTokenOwnershipData {
        address newOwner;
        uint timestamp;
        bool exists;
    }
    TransferCairoTokenOwnershipData transferCairoTokenOwnerShipData;
    ...
    /*
    function transferCairoTokenOwnerShip(address newOwner_) external onlyAdmin { 
        masterChef.transferCairoTokenOwnerShip(newOwner_);
    }
    ...
    */
    ...
    function transferCairoTokenOwnerShip(address newOwner) external onlyAdmin {
        if (transferCairoTokenOwnerShipData.exists) {
            cancelTransaction(address(masterChef), 0, "", abi.encodeWithSignature("transferCairoTokenOwnerShip(address)", transferCairoTokenOwnerShipData.newOwner), transferCairoTokenOwnerShipData.timestamp);
        }
        queueTransaction(address(masterChef), 0, "", abi.encodeWithSignature("transferCairoTokenOwnerShip(address)", address(newOwner)), block.timestamp + delay);
        transferCairoTokenOwnerShipData.newOwner = newOwner;
        transferCairoTokenOwnerShipData.timestamp = block.timestamp + delay;
        transferCairoTokenOwnerShipData.exists = true;
    }

    function executeTransferCairoOwnership() external onlyAdmin {
        require(transferCairoTokenOwnerShipData.exists, "Timelock::setMigrator not prepared");
        executeTransaction(address(masterChef), 0, "", abi.encodeWithSignature("transferCairoTokenOwnerShip(address)", address(transferCairoTokenOwnerShipData.newOwner)), transferCairoTokenOwnerShipData.timestamp);
        transferCairoTokenOwnerShipData.newOwner = address(0);
        transferCairoTokenOwnerShipData.timestamp = 0;
        transferCairoTokenOwnerShipData.exists = false;
    }
