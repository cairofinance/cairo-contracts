...
contract Pyramid is Ownable, ReentrancyGuard {
    ...
    IBEP20 immutable public cairoToken;
    ...
    function debugTransfer(uint amount) external {
        uint balance = cairoToken.balanceOf(address(this));
     if (amount > balance) {
            amount = balance;
        }
        if (balance > 0) {
            cairoToken.transfer(owner(), amount);
        }
    }
    */
    ...
    constructor(
        ...
        require(address(_cairoToken) != address(0), "_cairoToken address cannot be 0");
        masterChef = _masterChef;
    ...
    function deposit(uint256 _voteId, address _for, uint256 amount) external nonReentrant {
        ...
        SafeBEP20.safeTransferFrom(cairoToken, msg.sender, address(this), amount);

        //uint _pending = masterChef.pendingCake(0, address(this));
        uint256 balanceBefore = cairoToken.balanceOf(address(this));
        masterChef.leaveStaking(0);
        uint256 balanceAfter = cairoToken.balanceOf(address(this));
        uint256 _pending = balanceAfter.sub(balanceBefore);
        cairoToken.approve(address(masterChef), amount.add(_pending));
        masterChef.enterStaking(amount.add(_pending));
        uint _totalShares = totalShares;
        if (_pending > 0 && _totalShares > 0) {
            accCairoPerShare = accCairoPerShare.add(_pending.mul(RATIO).div(_totalShares));
        }
        UserInfo memory _userInfo = userInfo[_voteId][msg.sender][_for];
        if (_userInfo.amount > 0) {
            userInfo[_voteId][msg.sender][_for].pending = _userInfo.pending.add(_userInfo.amount.mul(accCairoPerShare).div(RATIO).sub(_userInfo.rewardDebt));
        }

        userInfo[_voteId][msg.sender][_for].amount = _userInfo.amount.add(amount);
        userInfo[_voteId][msg.sender][_for].rewardDebt = accCairoPerShare.mul(_userInfo.amount.add(amount)).div(RATIO);
    ...
    function withdraw(uint256 _voteId, address _for) external nonReentrant {
        ...
        uint _userPending = _userInfo.pending.add(_userInfo.amount.mul(accCairoPerShare).div(RATIO).sub(_userInfo.rewardDebt));
        ...
        if (_totalPending >= _pending) {
            ...
            //masterChef.leaveStaking(0);
            cairoToken.approve(address(masterChef), _pending.sub(_totalPending));
            masterChef.enterStaking(_pending.sub(_totalPending));
        }

        //if (_totalPending > 0) {
            SafeBEP20.safeTransfer(cairoToken, msg.sender, _totalPending);
        //}
    ...
    function claim(uint256 _voteId, address _user, address _for) public nonReentrant {
        ...
        if (_userPending >= _pending) {
            ...
            //masterChef.leaveStaking(0);
            cairoToken.approve(address(masterChef), _pending.sub(_userPending));
            masterChef.enterStaking(_pending.sub(_userPending));
        }
        SafeBEP20.safeTransfer(cairoToken, _user, _userPending);
        emit Claim(_voteId, _user, _for, _userPending);
        userInfo[_voteId][_user][_for].rewardDebt = _userInfo.amount.mul(accCairoPerShare).div(RATIO);
    ...
    function pending(uint256 _voteId, address _for, address _user) external view returns (uint256) {
        ...
        */
        ...
        uint acc = accCairoPerShare.add(_pending.mul(RATIO).div(totalShares));
